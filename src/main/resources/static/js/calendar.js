...
function main(){
    setMonthTitle(year,mon);
    makeCalendar(year,mon,getDayOfMon(mon,year));
    todoTitle.textContent = `What do you have to do on ${year}.${mon}.${currentDay} ?`;
    displayToDoOnDays();
}
...
Day.addEventListener('click',(event)=>{
    ...
    if(event.target.className!=='disabled'){
        clearEvent();
        todoTitle.textContent = `What do you have to do on ${year}.${mon}.${event.target.textContent} ?`;
...
reset.addEventListener('click',()=>{
    const result = prompt(`Do you really want to reset TODO on ${year} ${mon} ${DayOfChoice}? Enter (yes/no)`);
    const YMD = year+'-'+mon+'-'+DayOfChoice;
    if(result==='yes'){
...
allReset.addEventListener('click',()=>{
    const result = prompt(`Do you really want to clear all TODO? Enter (yes/no) not recomended`);
    if(result==='yes'){
        localStorage.clear();
        displayToDoOnDays();
    }
